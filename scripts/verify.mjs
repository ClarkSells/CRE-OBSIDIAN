...
for (const doc of ["README.md", "KNOWN_LIMITATIONS.md", "docs/ARCHITECTURE.md", "docs/SCHEMA.md", "docs/REALNEX_SYNC.md", "docs/FUTURE_AGENT_ENGINE.md", "docs/MANUAL_TESTS.md", "docs/ACCEPTANCE_CHECKLIST.md"]) check(`Documentation ${doc}`, await exists(path.join(root, doc)), doc);

const commandSource = await readFile(path.join(root, "main.ts"), "utf8");
const commandIds = ["initialize-vault", "open-command-center", "open-property-war-room", "open-owner-dossier", "open-relationship-graph", "open-deal-signal-radar", "open-comps-board", "open-investor-match", "open-realnex-queue", "create-relationship-edge", "rebuild-index", "validate-current-note", "export-realnex-csv", "generate-sample-dataset"];
...
const required = { property: 12, parcel: 12, entity: 10, person: 8, investor_profile: 8, tenant: 15, lease: 15, loan: 8, sale_comp: 15, lease_comp: 10, submarket: 8, deal_signal: 20, relationship_edge: 40, broker_activity: 10 };
